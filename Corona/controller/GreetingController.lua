local sceneName = GreetingController

-- Include
local view = require("view.GreetingView")
local model = require("model.GreetingModel")
...
hideGreeting = function()
    ...
    appData.firstSession = true

    local options = {effect = "crossFade", time = 350,}

    composer.removeScene( "controller.GreetingController", options )
    composer.gotoScene( "controller.ScheduleController", options )
end    
...
passengerCountdown = function ()
    ...
    if (view.passengerCountdown ~= nil) then
        ...
        if (difference < 0) then 
            countdownText = "Coming now!"
        else 
            countdownText = "Coming in "..difference.." minutes."    
        end  
    ...
    elseif (view.driverCountdown2 ~= nil ) then
        ...
        -- update the text 
        if (difference < 0) then 
            view.driverCountdown2.text = "You are late!"
        else      
            view.driverCountdown2.text = "Pick up your passenger in "..difference.." minutes."
        end     
    end
end
...
driverCountdown = function ()
    ...
    if (view.driverCountdown ~= nil) then
        ...
        if (difference < 0) then 
            countdownText = "Start driving now!"
        elseif difference < 300000 and difference > -60 then 
            countdownText = "Start driving in "..difference.." minutes"
            view.startButton.alpha = 1  
        else  
            countdownText = "Start driving at "..temp.hour..":"..temp.sec/60 
...
closeDrivingAlert = function()
    local alert = native.showAlert( "Do you want to end the trip?", "", { "NEI", "JA" }, closeDriving )
end
    ...
    -- end    
      ...
      if tonumber(os.date("%H")) <= 11 then
        if (appData.user.mode == "passenger") then
          ...
          view.transportRole.text = "Your passenger"
          local localHour = string.sub(appData.match.morning.pick_up_at, 12, 13)
          localHour = tonumber(localHour) + os.date("%z")/100
          localHour = tostring(localHour)
          local localMinute = string.sub(appData.match.morning.pick_up_at, 15, 16)
    ...
    if changing == true then
        ...
        view.tripChangeGroup.y = -3000 
                ...
                if view.tripRole.text == "passenger" then 
                    ...
                    print("PASSENGER 1")
                elseif view.tripRole.text == "driver" and appData.car.vehicle_id == "" then
                    appData.dummyTransports[i].vehicle = {}
                    appData.dummyTransports[i].vehicle.id = "0"
                    print("PASSENGER 2")
        ...
        if appData.dummyTransports[appData.i].created_at ~= "" then  
                    ...
                    if mode == "driver" and appData.car.vehicle_id == "" then
                        local alert = native.showAlert( "Attention!", "Register your car in settings and post your trip again, please.", { "OK", "" } )
                        return true
                    end 
...
moveScheduleSlide = function( event )
    ...
    elseif t.isFocus then
        ...
        elseif "ended" == phase or "cancelled" == phase then
            ...
            if math.abs(lastY - endY) > 2 then
                ...
                transition.to( view.scheduleSlide, { time = 2000, transition=easing.outCubic, y=targetY } )             
            end
        end
    end
    ...
    if view.routeMap2 ~= nil then
        view.routeMap2:toFront()
    end

    if view.routeMap1 ~= nil then
        view.routeMap1:toFront()
    end

    if view.greetingGroup ~= nil then
        view.greetingGroup:toFront()
    end    
...
setSwitches = function()
    if appData.appIsRunning == true then
...
onSwitch = function(event)
    if (event.phase == "ended") then
        ...
        if changing == true then 
                ...
                elseif view.tripRole.text == "driver" and appData.car.vehicle_id == "" then
                    appData.schedule[i].mode = 1
                    native.showAlert(  "Attention!", "Register your car in settings and post your trip again, please.", { "OK", "" })
                else    
        ...
        if (dayShift ~= 0) then
            ...
            elseif (dayPart == "a" and view.switches.afternoon[dayShift].isOn == true) then
                ...
                if i == 1 then  view.hint.afternoon[dayShift].text = "Før 21:00 i dag " end
...
morningMatchDownloaded = function(event)
    ...
    if (decoded.matches ~= nil) then
        ...
        if (decoded.matches[1] ~= nil) then
            ...
            if appData.time.morning.myDeparture > os.time() - 3600*2 then
            ...
            -- check TODAY ---------------------------------------------
                ...
                if (appData.match.morning.transport_entity_type_id == 2) then
                  print("-----------------------oooooo-----------------------")
                    ...
                    view.todayMorningTime.text = "pick up time: "..pickupTime
                    view.todayMorningRole.text = "Your driver"

                    if tonumber(os.date("%H")) <= 11 then
                        view.transportTime.text = "pick up time: "..pickupTime
                        view.transportName.text = appData.match.morning.person.firstname
                    ...
                    view.todayMorningRole.alpha = 1
                    view.todayMorningRole.text = "Your passenger"
...
afternoonMatchDownloaded = function(event)
    ...
    if (decoded.matches ~= nil) then
        ...
        if (decoded.matches[1] ~= nil) then
            ...
            if appData.time.afternoon.myDeparture > os.time() - 3600*2 then
                timer.performWithDelay( 5000, nextTrip, 1)
            ...
            print("this is the UTC: "..utcTime.." and this is the transport time: "..transportTime)
                ...
                if (appData.match.afternoon.transport_entity_type_id == 2) then
                    ...
                    view.todayAfternoonTime.text = "pick up time: "..pickupTime
                    view.todayAfternoonRole.text = "Your driver"
                    ...
                    view.todayAfternoonTime.text = "pick up time: "..pickupTime
                    view.todayAfternoonRole.text = "Your passenger"
...
tomorrowMorningMatchDownloaded = function(event)
    ...
    if (decoded.matches ~= nil) then
        if (decoded.matches[1] ~= nil) then
            ...
            local pickupTime = localHour..":"..localMinute
            view.hours.morning[1].text = "pick up time: "..pickupTime
        
            if (appData.match.tomorrowMorning.transport_entity_type_id == 2) then
                view.roles.morning[1].text = "Your driver"
            else
                view.roles.morning[1].text = "Your passenger"
            end
...
tomorrowAfternoonMatchDownloaded = function(event)
    ...
    if (decoded.matches ~= nil) then
        if (decoded.matches[1] ~= nil) then
            ...
            local pickupTime = localHour..":"..localMinute
            view.hours.afternoon[1].text = "pick up time: "..pickupTime
             
            if (appData.match.tomorrowAfternoon.transport_entity_type_id == 2) then
                view.roles.afternoon[1].text = "Your driver" 
            else
                view.roles.afternoon[1].text = "Your passenger"
            end
...
downloadMatches = function(event)
    ...
    for j=1, #appData.transports do
        if (appData.transports[j].starting_at ~= nil) then
            ...
            if (tomorrow == day and hour < 13) then
    ...
    for j=1, #appData.transports do
        if (appData.transports[j].starting_at ~= nil) then
            ...
            if (tomorrow == day and hour >= 12) then
...
enableMorningTransport = function()
    if (appData.match.morning.person ~= nil and
        morningTransportEnabled == false and
        tonumber(os.date("%H")) <= 11) then
        ...
        local limit = 60*60*2 -- 60 minutes before departure time
        ...
        if ((appData.time.morning.myDeparture - limit) < appData.time.morning.now)and
            (appData.time.morning.now < (appData.time.morning.otherDeparture + limit))then
...
enableAfternoonTransport = function()
    ...
    -- print("afternoon transport enabled 2")
        ...
        local limit = 60*60*2 -- 60 minutes before departure time
        -- limit = 60*60 -- 48 hours for testing
        ...
        if ((appData.time.afternoon.myDeparture - limit) < appData.time.now)and
            (appData.time.now < (appData.time.afternoon.otherDeparture + limit))then
...
nextTrip = function()
    ...
    if (appData.user.mode == "passenger" 
        ...
        if appData.match.afternoon.id ~= nil and appData.time.afternoon.myDeparture+3600 > os.time() - 3600*2 then 
            print("****************"..appData.time.afternoon.myDeparture)
            time = appData.time.afternoon.myDeparture 
            print("#------ 3       "..time)
        end 

        if appData.match.morning.id ~= nil and appData.time.morning.myDeparture+3600 > os.time() - 3600*2 then 
            time = appData.time.morning.myDeparture
            print("#------ 4       "..time)
        end

        local timeTable; local h; local m;
        ...
        -- show the info
        if (appData.user.mode == "driver") then
            view.timeInfo.text = "Start driving at: ".. timeValue
        else  
            view.timeInfo.text = "Pick up: ".. timeValue
        end   
        ...
        if (appData.user.mode == "driver") then
            ...
            if appData.match.afternoon.id ~= nil and appData.time.afternoon.myDeparture+3600*2 > os.time() then 
                myDeparture.ln = appData.location.afternoon.departure.ln
                myDeparture.lt = appData.location.afternoon.departure.lt
                myDestination.ln = appData.location.afternoon.pick_up.ln
                myDestination.lt = appData.location.afternoon.pick_up.lt
            ...
            if appData.match.morning.id ~= nil and appData.time.morning.myDeparture+3600*2 > os.time() then 
                myDeparture.ln = appData.location.morning.departure.ln
                myDeparture.lt = appData.location.morning.departure.lt
                myDestination.ln = appData.location.morning.pick_up.ln
                myDestination.lt = appData.location.morning.pick_up.lt
        ...
        elseif (appData.user.mode == "passenger") then
            ...
            if appData.match.morning.id ~= nil and appData.time.morning.myDeparture+3600*3 > os.time() then 
                myDeparture.ln = appData.location.morning.pick_up.ln
                myDeparture.lt = appData.location.morning.pick_up.lt
                myDestination.ln = appData.location.morning.pick_up.ln
                myDestination.lt = appData.location.morning.pick_up.lt
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
        ...
        view.greetingButton:addEventListener( "tap", hideGreeting )
    ...
    elseif ( phase == "did" ) then
        ...
        t14 = timer.performWithDelay( 36000000000000, showScene, 1)    
    end 

    view.sceneGroup.x = -1200
    appData.appIsRunning = true

    -- move some objects to the front
    view.scheduleSlide:toFront()
    view.greetingGroup:toFront()

    Runtime:addEventListener( "enterFrame", resetMap )
end
